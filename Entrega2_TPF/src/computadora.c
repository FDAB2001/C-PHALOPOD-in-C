...
extern int modo;
int contadorturnoinicial=0;

void dadocompu(int pos, int sumas) { //funcion que sirve para poder identificar a los dados de la compu
	if (decisiontomada == 1) {
		lamatriz[pos][sumas][0] = 2; //la compu se identificara por corchete
	}

	if (decisiontomada == 2) {
		lamatriz[pos][sumas][0] = 1; //la compu se identificara por parentesis
	}
...
void ponerDado(int jugada) {
	...
	if ((jugada > 1) && (jugada < 5)) {
		...
		if (lamatriz[jugada - 1][0][0] != '\0'
				&& lamatriz[jugada - 1][0][0] != 6)
			contador++;
		if (lamatriz[jugada + 1][0][0] != '\0'
				&& lamatriz[jugada + 1][0][0] != 6)
			contador++;
		if (lamatriz[jugada + 5][0][0] != '\0'
				&& lamatriz[jugada + 5][0][0] != 6)
			contador++;
...
//primero a la izquierda, depues a la derecha y despues abajo del dado
		...
		if (contador == 2) {
			...
			if (lamatriz[jugada + 1][0][0] != '\0'
					...
					&& (lamatriz[jugada + 1][0][0] + lamatriz[jugada - 1][0][0])
							< 7) {
				lamatriz[jugada][0][0] = lamatriz[jugada + 1][0][0]
						+ lamatriz[jugada - 1][0][0];

				int suma = lamatriz[jugada][0][0];
				dadocompu(jugada, suma); //estas dos lineas para lo que sirven es para identificar que es el dado de la compu
			...
			if (lamatriz[jugada + 1][0][0] != '\0'
					...
					&& (lamatriz[jugada + 1][0][0] + lamatriz[jugada + 5][0][0])
							< 7) {
				lamatriz[jugada][0][0] = lamatriz[jugada + 1][0][0]
						+ lamatriz[jugada + 5][0][0];

				int suma = lamatriz[jugada][0][0];
				dadocompu(jugada, suma);
	...
	if ((jugada == 6) || (jugada == 11) || (jugada == 16)) { //comienza
		...
		if (contador == 2
			...
			if (lamatriz[jugada + 5][0][0] != '\0'
					&& lamatriz[jugada - 5][0][0] != '\0'
					&& (lamatriz[jugada - 5][0][0] + lamatriz[jugada + 5][0][0])
							< 7) {
				lamatriz[jugada][0][0] = lamatriz[jugada + 5][0][0]
						+ lamatriz[jugada - 5][0][0];
		...
		if (contador == 3
				&& (lamatriz[jugada + 5][0][0] + lamatriz[jugada + 1][0][0]
						+ lamatriz[jugada - 5][0][0]) < 7) { //si el contador llega a 3 significa que hay 3 dados para que pueda sumar
			lamatriz[jugada][0][0] = lamatriz[jugada + 5][0][0]
					+ lamatriz[jugada + 1][0][0] + lamatriz[jugada - 5][0][0];
	...
	if ((jugada > 6 && jugada < 10) || (jugada > 11 && jugada < 15)
		...
		if (contador == 3
				&& (lamatriz[posi1][0][0] + lamatriz[posi2][0][0]
						+ lamatriz[posi3][0][0]) < 7) { //si el contador llega a 3 significa que hay 3 dados para que pueda sumar

			lamatriz[jugada][0][0] = lamatriz[posi1][0][0]
					+ lamatriz[posi2][0][0] + lamatriz[posi3][0][0];
		...
		if (contador == 4
				&& (lamatriz[posi1][0][0] + lamatriz[posi2][0][0]
						+ lamatriz[posi3][0][0] + lamatriz[posi4][0][0]) < 7) { //si el contador llega a 3 significa que hay 4 dados para que pueda sumar
...
turno = 1;
if(modo==2){
	if (decisiontomada== 2 && contadorturnoinicial==0) {
		srand(time(NULL));
		...
		if (posiocupada[1] == 6 || posiocupada[1] == 11
				|| posiocupada[1] == 16) {
			if (lamatriz[posiocupada[1] - 5][0][0] == '\0') {
				lamatriz[posiocupada[1] - 5][0][0] = 1;
				ponerDado(posiocupada[1] - 5);
		...
		if (posiocupada[1] == 10 || posiocupada[1] == 15
				|| posiocupada[1] == 20) {
			if (lamatriz[posiocupada[1] - 5][0][0] == '\0') {
				lamatriz[posiocupada[1] - 5][0][0] = 1;
				ponerDado(posiocupada[1] - 5);
		...
		if ((posiocupada[1] > 6 && posiocupada[1] < 10)
				|| (posiocupada[1] > 11 && posiocupada[1] < 15)
				|| (posiocupada[1] > 16 && posiocupada[1] < 20)) {
			if (lamatriz[posiocupada[1] + 1][0][0] == '\0') {
				lamatriz[posiocupada[1] + 1][0][0] = 1;
				ponerDado(posiocupada[1] + 1);
				return (turno);
