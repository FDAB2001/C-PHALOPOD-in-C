...
void ponerDado(int jugada){
	...
	if(jugada==1){
		...
		if(lamatriz[jugada+1][0][0]!='\0' && lamatriz[jugada+5][0][0]!='\0'  ){
			...
			int a=lamatriz[jugada+1][0][0]+lamatriz[jugada+5][0][0];

			lamatriz[jugada][lamatriz[jugada+1][0][0]+lamatriz[jugada+5][0][0]][0]=2;
...
int computadora(turno){
	...
	if(turnoinicial==2){
		...
		printf("Holaaa %d",lamatriz[numerorandom][0][0]);
		...
		printf("posociohn %d",posiocupada[1]);

		if(posiocupada[1]==1){

			if(lamatriz[2][0][0]=='\0'){

				ponerDado(posiocupada[1]);
